...
class TestModels(unittest.TestCase):
    ...
    def test_01_multiheaded_attention(self):
        ...
        C.model.n_layer = 6
        C.model.n_query_head = 4
        C.model.n_kv_head = 4
        C.model.n_embd = 16
        C.model.rope = False
        
        mha = model.CausalSelfAttention(config=C.model)
        mha.eval()

        x = torch.randn(2, 4, 16)
        output, *_ = mha(x)
        output_ref, *_ = mha.forward_ref(x)
        assert torch.allclose(output, output_ref, atol=1e-3), "Output does not match reference implementation"
